...
exports.registrar = function(req, res){
	objBD = BD.BD();
	objBD.connect();	
	registrar_documento = sanitize(req.body.registrar_documento).xss();registrar_documento = sanitize(registrar_documento).entityDecode();
	registrar_user = sanitize(req.body.registrar_user).xss();registrar_user = sanitize(registrar_user).entityDecode();
	registrar_correo = sanitize(req.body.registrar_correo).xss();registrar_correo = sanitize(registrar_correo).entityDecode();
	registrar_nombre = sanitize(req.body.registrar_nombre).xss();registrar_nombre = sanitize(registrar_nombre).entityDecode();
	registrar_phone = sanitize(req.body.registrar_phone).xss();registrar_phone = sanitize(registrar_phone).entityDecode();
		...
		objBD.query("INSERT INTO persona(User,Clave,Correo,Nombre,Telefono,tipo) VALUES ("+objBD.escape(registrar_user)+","+objBD.escape(pass)+","+objBD.escape(registrar_correo)+","+objBD.escape(registrar_nombre)+","+objBD.escape(registrar_phone)+",'p')", 
			function(err, result){
				if (err){
				  ...
				  objBD.query("INSERT INTO proveedor(ID_Persona,RIF) VALUES ("+result.insertId+","+objBD.escape(req.body.registrar_documento)+")", 
				  	function(err2, result2){
							if (err2){
						  	res.write('1');
							  res.end();
...
exports.login = function(req, res){
	objBD = BD.BD();
	objBD.connect();
	login_user = sanitize(req.body.login_user).xss();login_user = sanitize(login_user).entityDecode();
	if(login_user && req.body.user_pass){
		pass= crypto.createHash('sha256').update(req.body.user_pass).digest("hex");
		pass= pass.substr(0,1)+"u"+pass.substr(2,pass.length/2)+"se"+pass.substr(pass.length/2)+"r";
		objBD.query("SELECT ID_Persona, User, tipo FROM persona WHERE User = "+ objBD.escape(login_user) +" AND Clave = "+ objBD.escape(pass) +" OR Correo = "+ objBD.escape(req.body.login_user) +" AND Clave = "+ objBD.escape(pass) +"",
...
exports.pictureUpload = function(req, res){
	if (req.session.user){
		var tmp_path = req.files.photoimg.path;
...
exports.pictureSN = function(req, res){
	if (req.session.user){
		id_social_network_fort = sanitize(req.body.id_social_network_fort).xss();id_social_network_fort = sanitize(id_social_network_fort).entityDecode();
		if(id_social_network_fort=="f")
			url_SN = "http://graph.facebook.com/"+req.body.id_social_network+"/picture?type=large";
		else if(req.body.id_social_network_fort=="t")
			url_SN = "http://api.twitter.com/1/users/profile_image?screen_name="+req.body.id_social_network.substr(1)+"&size=original";
...
exports.pictureSave = function(req, res){
	if (req.session.user){
		w_resize = sanitize(req.body.w_resize).xss();w_resize = sanitize(w_resize).entityDecode();
		h_resize = sanitize(req.body.h_resize).xss();h_resize = sanitize(h_resize).entityDecode();
		w = sanitize(req.body.w).xss();w = sanitize(w).entityDecode();
		h = sanitize(req.body.h).xss();h = sanitize(h).entityDecode();
		x1 = sanitize(req.body.x1).xss();x1 = sanitize(x1).entityDecode();
		y1 = sanitize(req.body.y1).xss();y1 = sanitize(y1).entityDecode();
